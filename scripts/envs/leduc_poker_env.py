...
class RewardCalculator:
    """
    Per-step shaping (clipped to ``SHAPING_REWARD_CLIP``) + clipped terminal env reward.

    Mirrors the old (commit 7ae2e8a8) ``_score_action_quality`` shaping topology:
    small soft-shaping deltas per step, accumulated and clipped at episode end,
    summed with the (separately clipped) terminal env reward.
    """
    ...
    @staticmethod
    def _hand_strength(gs: "GameState") -> float:
        """Normalised [0,1] hand strength, mirroring the old _hand_strength heuristic.

        rank_val uses J=0, Q=1, K=2 (matching old CARD_RANK).
        """
        rank_val = gs.private_card_rank - 1  # J=0, Q=1, K=2
        if gs.has_pair:
            return 0.8 + 0.1 * rank_val           # J-pair=0.8, Q-pair=0.9, K-pair=1.0
        if gs.public_card_rank is None:
            return rank_val / 2.0                 # pre-flop: J=0.0, Q=0.5, K=1.0
        return rank_val / 4.0                     # post-flop no pair: J=0.0, Q=0.25, K=0.5

    def calculate_step_reward(self, gs: "GameState | None", action_str: str) -> float:
        """Per-step shaping. Does NOT include env_reward or invalid penalty."""
        if gs is None:
            return 0.0

        strength    = self._hand_strength(gs)
        action_type = self._classify_action(action_str)
        can_raise   = gs.can_raise
        reward      = 0.0

        if action_type == "fold" and strength >= 0.6:
            reward -= self.FOLD_WITH_STRONG_HAND_PENALTY
        if action_type == "raise" and strength >= 0.7:
            reward += self.VALUE_BET_BONUS
        if action_type == "raise" and gs.round == 1 and strength <= 0.5:
            reward += self.BLUFF_QUALITY_BONUS
        if action_type == "call" and gs.round == 2 and strength >= 0.7 and can_raise:
            reward -= self.PASSIVE_WITH_STRONG_PENALTY
        if action_type == "fold" and gs.round == 2 and strength < 0.3:
            reward += self.FOLD_WEAK_HAND_BONUS
    ...
    def finalize_reward(
        ...
        clipped     = max(-self.SHAPING_REWARD_CLIP, min(self.SHAPING_REWARD_CLIP, accumulated))
        terminal    = max(-self.TERMINAL_REWARD_CLIP, min(self.TERMINAL_REWARD_CLIP, final_env_reward))
...
def _ensure_initialized(trainer) -> None:
    ...
    if _log_rank == "all" or str(rank) == _log_rank:
        print(
            ...
            f"final_max_turn=8, rollouts_per_stage={trainer.args.rollouts_per_stage}"
        )
    ...
    *,
    ...
    while not done and turn_number < current_max_turn:
        ...
        try:
            ...
            action_str = ""
        step_shaped = calculator.calculate_step_reward(prev_gs, action_str)
        rewards.append(step_shaped)

        messages.append({"role": "user", "content": observation})
        turn_number += 1

    train_reward = calculator.finalize_reward(rewards, final_reward, episode_reward)
    _metric_line = (
