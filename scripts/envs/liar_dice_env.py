...
class RewardCalculator:
    """
    Per-step shaping (clipped per step) + clipped terminal env reward.

    Mirrors the old (commit 7ae2e8a8) Liar's Dice shaping topology:
    bid plausibility + challenge-decision shaping per step, each step clipped
    to ``PER_STEP_SHAPING_CLIP``, accumulated and clipped at episode end to
    ``SHAPING_REWARD_CLIP``, then summed with the (separately clipped) terminal
    env reward.
    """
    ...
    def _bid_rank(bid: "Bid") -> int:
        return bid.quantity * 6 + bid.face

    def _score_bid_plausibility(self, gs: "GameState", taken: "Action") -> float:
        """Continuous shaping based on the taken bid's truth probability."""
        if taken.bid is None:
            return 0.0
        truth_probability = float(taken.prob)
        signed = 2.0 * truth_probability - 1.0
        reward = signed * (self.BID_PLAUSIBILITY_BONUS if signed >= 0 else self.BID_PLAUSIBILITY_PENALTY)

        if gs.current_bid is not None:
            jump = self._bid_rank(taken.bid) - self._bid_rank(gs.current_bid)
            if jump <= 2:
                reward += 0.01
            elif jump >= 7:
    ...
    def _score_challenge_decision(self, gs: "GameState", chose_liar: bool, taken_bid: "Bid | None") -> float:
        """Reward correct-vs-overconfident challenges; penalise missed challenges."""
        if gs.current_bid is None:
            return 0.0

        current_truth_prob = float(bid_probability(gs.current_bid, gs))  # P(opponent's last bid is true)

        reward = 0.0
        if chose_liar:
            if current_truth_prob <= 0.20:
                reward += self.CORRECT_CHALLENGE_BONUS * (
                    1.0 + self._clamp((0.20 - current_truth_prob) / 0.20, 0.0, 1.0)
                )
            elif current_truth_prob >= 0.60:
                reward -= self.OVERCONFIDENT_CHALLENGE_PENALTY
        elif taken_bid is not None:
            if current_truth_prob <= 0.20:
                reward -= self.PASS_MISSED_CHALLENGE_PENALTY * (
                    1.0 + self._clamp((0.20 - current_truth_prob) / 0.20, 0.0, 1.0)
                )
            elif current_truth_prob >= 0.55:
                reward += 0.01
        return reward

    def calculate_step_reward(self, gs: "GameState | None", taken: "Action | None") -> float:
        """Per-step shaping. Does NOT include env_reward or invalid penalty.

        Returns the per-step shaping clipped to ``PER_STEP_SHAPING_CLIP``.
        """
        if gs is None or taken is None:
            return 0.0
        bid_part        = self._score_bid_plausibility(gs, taken)
        challenge_part  = self._score_challenge_decision(gs, taken.is_liar, taken.bid)
        per_step        = bid_part + challenge_part
        return self._clamp(per_step, -self.PER_STEP_SHAPING_CLIP, self.PER_STEP_SHAPING_CLIP)

    def finalize_reward(
        self,
        shaping_rewards: list[float],
        final_env_reward: float,
        invalid_penalty_sum: float = 0.0,
    ) -> float:
        accumulated = sum(shaping_rewards) + invalid_penalty_sum
        clipped     = self._clamp(accumulated, -self.SHAPING_REWARD_CLIP, self.SHAPING_REWARD_CLIP)
        terminal    = self._clamp(final_env_reward, -self.TERMINAL_REWARD_CLIP, self.TERMINAL_REWARD_CLIP)
        return terminal + clipped
...
def _ensure_initialized(trainer) -> None:
    ...
    if _log_rank == "all" or str(rank) == _log_rank:
        print(
            ...
            f"final_max_turn=20, rollouts_per_stage={trainer.args.rollouts_per_stage}"
        )
...
def _run_episode(
    ...
    *,
    ...
    while not done and turn_number < current_max_turn:
        ...
        # --- Reward calculation (per-step shaping; terminal applied at episode end) ---
        last_action_prob = 0.0
        if is_invalid:
            invalid_penalty_sum -= RewardCalculator.INVALID_ACTION_PENALTY
        else:
            try:
                previous_game_state = game_state_history[-1]
                action_id, _has_extra_text = _parse_action_id(action_to_send)
                ...
                step_shaped = calculator.calculate_step_reward(previous_game_state, taken_action)
                rewards.append(step_shaped)

        turn_number += 1

    # --- Final reward: clipped shaping + clipped terminal env reward ---
    train_reward = calculator.finalize_reward(rewards, final_reward, invalid_penalty_sum)
