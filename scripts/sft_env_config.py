...
SFT_ENV_SIZE_CONFIG: dict[str, dict] = {
    "0_1_b":  {"lr": 1e-4,   "distributed": "ddp", "gpu_count": 1, "batch_size": 140, "use_lora": True},
    "1_2_b":  {"lr": 1e-4,   "distributed": "ddp", "gpu_count": 1, "batch_size": 100, "use_lora": True},
    "2_4_b":  {"lr": 7.5e-5, "distributed": "ddp", "gpu_count": 1, "batch_size": 48,  "use_lora": True},
    "4_5_b":  {"lr": 7e-5,   "distributed": "ddp", "gpu_count": 2, "batch_size": 40,  "use_lora": True},
    "5_9_b":  {"lr": 3.5e-5, "distributed": "ddp", "gpu_count": 2, "batch_size": 28,  "use_lora": True},
...
def get_training_json(train_info: dict) -> dict:
    ...
    env_name = train_info.get("dataset_type", {}).get("environment_name", "liars_dice")
    dataset_path = f"/workspace/scripts/datasets/sft_env_{task_id}"

    _ENV_GENERATE_ARGS = {
        "gin_rummy":   {"num_games": 8000,   "max_turn": 200},
        "leduc_poker": {"num_games": 200000, "max_turn": 10,
    ...
    if train_info.get("find_lk_lr"):
        ...
        if lr is not None:
            print(f"Using lr from architecture config: {lr}", flush=True)
    ...
    if _env_args.get("sample_by_score"):
        generate_cmd += f" --sample-by-score --score-power {_env_args['score_power']}"
    elif _env_args.get("wins_only"):
        generate_cmd += " --wins-only"
